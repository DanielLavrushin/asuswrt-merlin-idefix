...
- FIXED: **Safari disconnected the terminal constantly** ([#11](https://github.com/DanielLavrushin/asuswrt-merlin-idefix/issues/11)). Safari freezes a page into its back/forward cache when you switch tabs or navigate elsewhere in the router UI, and closes the WebSocket with `WebSocket is closed due to suspension.`
...
- SECURITY: **Port 8787 was open to the WAN** ([#12](https://github.com/DanielLavrushin/asuswrt-merlin-idefix/issues/12)).
...
- SECURITY: **GitHub release notes were rendered as unsanitized HTML** ([#17](https://github.com/DanielLavrushin/asuswrt-merlin-idefix/issues/17)).
- SECURITY: **The terminal token is no longer put in the WebSocket URL** ([#16](https://github.com/DanielLavrushin/asuswrt-merlin-idefix/issues/16)). It now travels in the `Sec-WebSocket-Protocol` header — the only header a browser lets a WebSocket set.
- SECURITY: **A terminal token is now spent on the connection it opens.** Previously it stayed valid for its full two-minute lifetime and could be used any number of times, and because it names the session owner, anyone who captured one off a plaintext connection could re-attach to your live shell rather than merely open a new one. Each token is now accepted exactly once.
- SECURITY: Tokens now carry a random nonce, so two minted in the same second are no longer identical. The signature previously covered only the client id and a whole-second timestamp, which meant two terminal tabs could be issued the very same token.
- SECURITY: The server now refuses to start if `sec.key` is not valid hex or holds less than 32 bytes. A malformed key was previously decoded into an empty HMAC key and every token signed with it would have been forgeable by anyone.
- SECURITY: **The terminal now refuses WebSocket handshakes from pages on another site** ([#15](https://github.com/DanielLavrushin/asuswrt-merlin-idefix/issues/15)). A browser that sends an `Origin` must now name the same host the terminal is answering on; the port may differ, since the router UI and the terminal do not share one. This is defence in depth rather than a fix for a way in: the terminal has always required a signed token that no other site can obtain or forge, so a page you visited elsewhere could not have opened a shell. The server's own origin check stayed off because it compares ports too and would have rejected every legitimate connection.
